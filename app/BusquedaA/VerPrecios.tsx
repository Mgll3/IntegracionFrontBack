...
const VerPrecios: NextPage<VerPreciosProps> = ({ originCity, destinationCity, adults, child, infants, tripType, departureDate, arrivalDate}) => {
    ...
    //Conectarse al unico Endpoint Backend (Toda la Api Cambio en el Sprint3)
    const fetchData = async () => {
        // Url base 
        const url: string = `${baseUrl}/v3/api/fares/search`;
        //Ingreso los datos para le petición
        const data = {
            departureDate: `${departureDate}`,
            //Duplico el departure Date por la forma en que recibe y busca la información el back, pues no tiene vuelo regreso
        ...
        fetchDataFlights(url, data);
        
    };


    //Obtener los datos de la Api - Re uso el codigo para futuro two Ways
    const fetchDataFlights = async (url: string, data: object) => {
        
        fetch(url, {
            // Envio Peticióón tipo Post
            method: 'POST', 
            headers: {
                'Content-Type': 'application/json' // Tipo de contenido a enviar
            },
            // Convierto los datos JSON
            ...
            .then((jsonData) => {
                ...
                // Utilizo solo los datos que el front esta usando
                const flights = jsonData.map((flightData: any) => {
                    const { flight: { departureTime: hora_ida, arrivalTime: hora_vuelta }, price: precio} = flightData;
                    return { hora_ida, hora_vuelta, precio };
                });
                //Aplico los datos a la vista
                setFlights(flights);
                ...
                <div className="flight-header-container">
                    <div className="flight-header">
                        ...
                        <div className="flight-header-filter">
                            ...
                            <span id = "resultSpan" >Recomendado:</span>
